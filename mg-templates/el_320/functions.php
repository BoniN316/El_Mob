...
function kupit_func($args) {
  $output = '
        <div class="pbuttons">
                <a class="pbutton bttech" target="_blank" href="https://elakor-floor.ru/zakazat-konsultatsiyu" title="Заявка">Заказать</a>
              </div>
    ';
    return  $output;
}
...
$l=3;
    ...
    $prices = '<p style="text-align:center; color:red;"> Таблица цен по объему приобретения</p><br>
        <table id="tblCn">

<tr><td><strong>10-39кг</strong></td><td><span itemprop="highPrice">'.mb_substr($prod_prices['10'],0,strlen(($prod_prices['40']))-$l).'</span>p.</td></tr>
            <tr><td><strong>40-199кг</strong></td><td>'.$prod_prices['40'].'</td></tr>
            <tr><td><strong>200-499кг</strong></td><td>'.$prod_prices['200'].'</td></tr>
            <tr><td><strong>500-1тн</strong></td><td>'.$prod_prices['500'].'</td></tr>
            <tr><td><strong>1-3тн</strong></td><td>'.$prod_prices['1000'].'</td></tr>
            <tr><td><strong>более 3тн</strong></td><td><span itemprop="lowPrice">'.mb_substr($prod_prices['3000'],0,strlen(($prod_prices['3000']))-$k).'</span>'.$star.'</td></tr>
        </table>
    ';
...
if (isset($args['id']) && $args['id'] != '') {
  ...
  foreach ($mxlwprod_prices as $key => $value) {
   ...
   $ipos= strrpos ( $mxlwprod_prices['12'], '*');
...
$k=3;
  ...
  $mxlwprices =mb_substr($mxlwprod_prices['10'],0,strlen(($mxlwprod_prices['10']))-$k);
...
//-------------------------------Получение цены продукта по количеству //заказа-----------------------
function truProdPrice($id,$kolvo) {
...
$filename = 'price.csv';
  ...
  if(!file_exists('uploads/'.$filename)) return 316;

  $f = fopen(SITE.'/uploads/'.$filename, "r") or die("Ошибка!");
  for ($i=0; $data=fgetcsv($f,1000,";"); $i++) {
      if($i != 0) {
          $prod_id = iconv('windows-1251', 'utf-8', $data[0]);
          if($id != 0 && $id != $prod_id) continue;

          $prices[$i]['10'] = iconv('windows-1251', 'utf-8', $data[4]);
          ...
          $prices[$i]['id'] = iconv('windows-1251', 'utf-8', $data[0]);// здесь ID
...
if($kolvo<10){$kolvo=10;}

if ($kolvo>=10 && $kolvo<=39){$d='10';}
if ($kolvo>=40 && $kolvo<=199){$d='40';}
if ($kolvo>=200 && $kolvo<=499){$d='200';}
if ($kolvo>=500 && $kolvo<=1000){$d='500';}
if ($kolvo>1000 && $kolvo<=3000){$d='1000';}
if ($kolvo>3000 ){$d='3000';}
...
mgAddShortcode('truKolProdPrice', array(__CLASS__, 'truProdPrice'));
...
/*  if(!file_exists(SITE.'/uploads/'.$filename)) return $products;*/
