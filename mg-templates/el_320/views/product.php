...
<div class="product-top-block clearfix">
    <?php $mat = in_array($data[cat_id], array(2,5,8,6,7));
    	...
    	<div class="buy-block-bottom clearfix">
            <div class="buy-left">
               ...
               <input type="hidden" name="inCartProductId" value="<?php echo $data["id"]; ?>">
                    ...
                    <a id="kupOpisT" class="pbutton btgreen" href="<?php echo SITE.'/catalog?inCartProductId='.$data["id"]; ?>">Купить</a>
...
<td class="fotabL">
                    ...
                    <?php
                      ...
                      foreach ($data['images_product'] as $image) {
                        echo '<a class="popup-product-image" href="'.SITE.'/uploads/'.$image.'" title="'.$data['images_title'][$i].'" rel="product-top-gal" title="'.$data['images_title'][$i].'"><img style="width: 157%;" src="'.mgImageProductPath($image, $data['id'], 'big').'" title="'.$data['images_title'][$i].'" alt="'.$data['images_alt'][$i].'"></a>';
                        $i++;
                      }
...
<td class="fotabR">
    ...
    <li><a href="#tab_Content3" class="tab" id="DContent3">Применение</a></li>
    <li><a href="#tab_Content4" class="tab" id="DContent4">Сертификаты</a></li>
    <li><a href="#tab_Content5" class="tab" id="DContent5">Цены</a></li>
  </ul>
...
<td class="potabL">
                    ...
                    <?php
                      $i = 0;
                        ...
                        echo '<img class="imgPokTop" style="width: 100%;" src="'.mgImageProductPath($data['images_product'][$i] , $data['id'], 'big').'" title="'.$data['images_title'][$i].'" alt="'.$data['images_alt'][$i].'">';
...
<!-- brend IN-->
 <span  id="brd" itemprop="brand" style="color:#FFFFFF;>'Элакор'</span> <span itemprop="model" style="color:#FFFFFF;"><?php echo $data['code'].print_r($data['id']) ?></span>
...
<?php
                ...
                /* $mat = in_array($data[cat_id], array(22, 23, 24, 25,41));*/
                  ...
                  if ($mat) {
                  ?>
<div id="rzdMat" class="material"></div>
 ...
 <BR><BR>
<div id="rzdPokr"></div>
  <div id="PokKTp" class="product-top-in product-top-right product-prices">Стоимость комплекта материалов "Элакор" рассчитывается, исходя из площади, условий объекта  и дополнительных требований заказчика </div>
                   ...
                   /*  $mat = in_array($data[cat_id], array(22, 23, 24, 25,41));*/
                      ...
                      if ($mat) {
                      ?>
  <div id="abCen" class="product-prices" ><p>Розничная цена за 1 кг с учетом <br> тары и НДС.<br> Скидки в разделе Цены.</p>
...
<form method="POST" class="property-form actionView" data-product-id="<?php echo $data["id"]; ?>">
               ...
               <input type="hidden" name="inCartProductId" value="<?php echo $data["id"]; ?>">
                    ...
                    <a id="kupOpis" class="pbutton btgreen" href="<?php echo SITE.'/catalog?inCartProductId='.$data["id"]; ?>">Купить</a>
                  <input type="submit" name="buyWithProp" style="display:none">
                </form>
...
<span itemprop="seller">Производитель:"ТэоХим"</span>
 ...
 <BR><BR>
  ...
  <div id="PokKT" class="product-top-in product-top-right product-prices">Стоимость комплекта материалов "Элакор" рассчитывается, исходя из площади, условий объекта  и дополнительных требований заказчика </div>
                       ...
                       <?php }
                        ?>
 <!-- product-top-block -->
                     <?php if ($mat) {echo '<div class="abCen"> <p>Цена указана в рублях за 1 кг с учетом НДС и тары.</p></div>';} ?>
 ...
 <?php
                     ...
                     /* $mat = in_array($data[cat_id], array(22, 23, 24, 25,41));*/
                      ...
                      if ($mat==false) {
                      ?>
<a id="ZakPokr" class="pressed-button" href="/">Заказать покрытие</a>
<?php } ?>
</div>