...
$(document).ready(function() {
  $('script').each(function() {
    if ($(this).attr('src')) {
      ...
      $(this).attr('src').replace(/(\w+)(?:=([^&]*))?/g, function(a, key, value) {
        ...
        if (key === 'mgBaseDir') {
          if (protocol == 'http') {
            mgBaseDir = 'https://' + convertPunicode(value);
          } else {
  ...
  $('.product-wrapper .variants-table').each(function() {
    ...
    if ($(this).find('td input:checked').data('count') != 0 && $(form).find('.buy-container a.addToCart').length==0) {
      ...
      var buttonbuy = '<a href="https://'+mgBaseDir+'/catalog?inCartProductId='+id+'" class="addToCart product-buy" data-item-id="'+id+'">'+namebutton+'</a>';
      $(form).find('.buy-container .hidder-element ').append(buttonbuy);
    }
  });
  $('body').on('click', '.product-cart .cart_form .amount_change a button#krzRefr', function(event){
     // Пересчет цены и количества
    var request =  $(this).parents('.cart-wrapper').find('form').formSerialize();
    console.log("priexaliREFRTarget  ");
  console.log("priexaliREFRRequest  ", request);
...
function updateCartCount(request){
        console.log('function updateCartCount)');
$.ajax({
type: "POST",
  url: mgBaseDir + "/cart",
  data: "refresh=10&count_change=1000&" + request,
  dataType: "json",
  cache: false,
  success: function(response) {
    if (response.data) {
