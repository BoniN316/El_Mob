...
// var_dump($_POST['itemId']);
    ...
    if (!empty($_REQUEST['refresh'])) {
      ...
      foreach ($refreshData as $key => $val) {// записываем в val каждый элеиент переборо + в $key- значения ключей
        $id = '';
        if ('item_' == substr($key, 0, 5)) {//ищем строку с item_
          $id = substr($key, 5);//вытаскиваем ID
          // Находим propertyReal для текущего ID продукта.
          $propertyReal = array();//объявляем
          $variantId = array();//объявляем
          if (!empty($_SESSION['cart'])) {//проеряем существование значений
            foreach ($_SESSION['cart'] as $item) {// перебираем и присваиваем $item значения переборов
              if ($item['id'] == $id) {//при попадании на свой ID
                $propertyReal[] = $item['propertyReal'];//записываем свойства товара(полимер, температура, стойкость и т.п.)
                $variantId[] = $item['variantId'];// 0 - без вариантов
          ...
          if (!empty($val)) {
            ...
            foreach ($val as $k => $count) {
              $propertySetId = $refreshData['property_' . $id][$k];//6 пока при к=0
      ...
      foreach ($_SESSION['cart'] as $key => $item) {
        if ($item['propertySetId'] == $_POST['propertySetId'] && $item['id'] == $_POST['inCartProductId']) {
          $_SESSION['cart'][$key]['count'] = (is_numeric($_REQUEST['amount_input'])) ?
                  intval($_REQUEST['amount_input']) : 10;//Установка начального количества товаров при покупке
        }
      }
      $response = array(
        'status' => 'success',
...
//echo 'delllllcontrollercart/////',$_POST['itemId'], $property, $_POST['variantId']; все приходит
