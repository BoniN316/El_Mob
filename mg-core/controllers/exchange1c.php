...
//	MG::loger(print_r($_GET, true));
//	MG::loger(print_r($_POST, true));
//	MG::loger(print_r($_SERVER, true));
    ...
    if ($xml && ($filename == 'import.xml' || $filename == 'import0_1.xml')) {
      ...
      foreach ($xml->Каталог->Товары[0] as $item) {
        ...
        if ($row = DB::fetchAssoc($res)) {
          ...
          if (UPDATE_INFO_1C != 'UPDATE_INFO_1C' && UPDATE_INFO_1C != 0) {          
            // При импорте данных из 1с, не перезаписывать, картинки и описание товара из 1с
            unset($dataProd['description']);
            unset($dataProd['image_url']);
          }
        ...
        if(!empty($realImgPath)){
          foreach($realImgPath as $cell=>$image){
            if (!empty($image) && is_file($image)) {
              $bigImg = $upload->_reSizeImage('70_'.$imageUrl[$cell], $realDocumentRoot.$sep.$image, $widthPreview, $heightPreview, $resizeType, 'uploads'.$sep.$addPath.'thumbs'.$sep);
              $smallImg = $upload->_reSizeImage('30_'.$imageUrl[$cell], $realDocumentRoot.$sep.$image, $widthSmallPreview, $heightSmallPreview, $resizeType, 'uploads/'.$addPath.'thumbs/');
