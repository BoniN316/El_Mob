...
class Controllers_Payment extends BaseController {
  ...
  public function payanyway($paymentID, $paymentStatus) {
    ...
    if ('success' == $paymentStatus) {   
      $orderInfo = $order->getOrder(" id = " . DB::quote(intval($_GET['MNT_TRANSACTION_ID']), 1));
      ...
      if ($_POST['MNT_COMMAND'] == 'CHECK' && empty($_POST['MNT_OPERATION_ID'])) {
        ...
        $alg = $paymentInfo[3]['value'];
        $sign = hash($alg,$_POST['MNT_COMMAND'] . $paymentInfo[0]['value'] . $paymentOrderId .  $summ . $currency . $testmode . $paymentInfo[1]['value']);
      
		
		
        if ($sign == $_POST['MNT_SIGNATURE']) {
          $signNew = hash($alg,'402' . $paymentInfo[0]['value'] . $paymentOrderId . $paymentInfo[1]['value']);
        ...
        $currency = (MG::getSetting('currencyShopIso') == "RUR") ? "RUB" : MG::getSetting('currencyShopIso');
        $alg = $paymentInfo[3]['value'];
        $sign = hash($alg,$_POST['MNT_COMMAND'] .$paymentInfo[0]['value'] . $paymentOrderId . $_POST['MNT_OPERATION_ID'] . $summ  . $currency . $testmode . $paymentInfo[1]['value']);
  ...
  private function liqpay($paymentID, $paymentStatus){ 
    ...
    if('result' == $paymentStatus && isset($_POST)){
      ...
      $privateKey = $paymentInfo[1]['value'];
