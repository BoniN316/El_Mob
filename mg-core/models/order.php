  ...
  public function isValidData($arrayData, $reqire = array('email','phone','payment'),$createUser = true , $error = null) {
    ...
    if (CAPTCHA_ORDER == '1'){
      ...
      if (empty($arrayData['capcha']) || (strtolower($arrayData['capcha']) != strtolower($_SESSION['capcha']))) {
       ...
       // проверка в налии товаров, пока человек оформляет заказ -товар может купить другой пользователь
          ...
          if ($prod = DB::fetchArray($res_var)){
            if ($prod['count'] >=0 && $prod['count'] < $_SESSION['cart'][$key]['count']){
              if ($prod['count'] == 0) {
                ...
                $error .= "<p>Товар ".$prod['title'].' '.$prod['title_variant']." доступен в количестве ".$prod['count']." шт. Для оформления заказа измините количество в корзине.</p>";
              }              
            }            
          }          
        }else{  
          ...
          if ($prod = DB::fetchArray($res_pr)){
            if ($prod['count'] >=0 && $prod['count'] < $_SESSION['cart'][$key]['count']){
              if ($prod['count'] == 0) {
                ...
                $error .= "<p>Товар ".$prod['title']." доступен в количестве ".$prod['count']." шт. Для оформления заказа измините количество в корзине.</p>";
              }   
            }            
          }   
        }
  ...
  public function getParamArray($pay, $orderId, $summ) {
    ...
    if (!empty($jsonPaymentArray)) {
      ...
      if (5 == $pay) { // Для robokassa добавляем сигнатуру.
        ...
        $paramArray['sign'] = hash($alg,$paramArray[0]['value'].":".$summ.":".$orderId.":".$paramArray[1]['value']);
      }
      if (9 == $pay) { // Для payanyway добавляем сигнатуру.	    
        ...
        $alg = $paramArray[3]['value'];        
        $paramArray['sign'] = hash($alg, $paramArray[0]['value'].$orderId.$summ.$currency.$testmode.$paramArray[1]['value']);
      }
      if(15 == $pay){
