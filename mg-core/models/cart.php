...
class Models_Cart {
   ...
   * @return bool
   */
  public function addToCart($id, $count = 10, $property = array('property' => '', 'propertyReal' => ''), $variantId = null) {
      ...
      foreach ($_SESSION['cart'] as $key => $item) {
        if ($variantId > 0) {
          ...
          if ($id == $item['id'] && $property == $item['property']) {
            ...
            unset($_SESSION['cart'][$key]);
            //echo 'delllllmodelcart*************';при удалении обнуляет переменную по ид
...
//echo '#1#',$product;
    ...
    $product['price']=truProdPrice($prod,$_SESSION['cart'][$product['keyInCart']]['count'] );
  ...
  public function getItemsCart() {
    ...
    $itemPosition = new Models_Product();
    $totalSumm = 0;
    if (!empty($_SESSION['cart'])) {//список товаров в корзине
      ...
      $productsId = array();
      foreach ($_SESSION['cart'] as $key => $item) {
        if (!empty($item['variantId'])) {//дополнения к основному товару =0 как правило
          $variantsId[] = intval($item['variantId']);
        }
        if (!empty($item['id'])) {
          $productsId[] = intval($item['id']); // вытаскивает id товара
        }
      }
      $products_all = array();
      $variants_all = array();
      ...
      foreach ($_SESSION['cart'] as $key => $item) {
        ...
        $product['price'] = $this->customPrice(array(
          ...
          if ($product['countInCart'] > 0) {
           ...
           $product['price'] = truProdPrice($product['id'],$item['count']);
