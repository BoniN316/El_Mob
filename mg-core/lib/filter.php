 ...
 public function getFilterSql($data, $sorter = array(), $insideCat = true) {
    ...
    foreach ($data as $k => $v) {
      ...
      if ((!empty($v) && $v != 'null') || $v === 0 || $v === '0') {
        ...
        if (is_array($v) && count($v) == 2 && $v[1]=='like') {
          $devide = ' like ';
          $v = DB::quote('%'.$v[0].'%');
        }  // если в значении передан маcсив двух значений, значит будет моделироваться оператор BETWEEN
    ...
    if (!empty($sorter)) {
      if (!empty($sorter[0])) {
        ...
        $where .= " ORDER BY ".DB::quote($sorter[0])." ".DB::quote($sorter[1]);
      }
    }
      ...
      if(!empty($values)){
        ...
        if($prop['type_filter']=='checkbox'){
          ...
          foreach ($values as $value) {          
           ...
           //  echo ($this->accessValues[$idProp].' -> '.$value.'<br>');
              if(!empty($this->accessValues[$idProp]) && in_array($value, $this->accessValues[$idProp])){     
                $value = htmlspecialchars($value);
                $html .= ' <li style="'.$style.'"><label class="active"><input  type="checkbox" name="prop['.$idProp.'][]" value="'.$value.'" '.$checked.'  class="mg-filter-prop-checkbox"/>'.$value.'</label>'.$viewAll.'</li>';
