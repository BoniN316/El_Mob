  ...
  $('.send-ring-button').click(function() {
    ...
    $.ajax({
      ...
      success: function(response) {
          console.log("priexali  ", response.status );
        if (response.status != 'error') {
            $('span.txtopermsg').css('display','inline-block');
          $('span.txtopermsg').text('Ваша заявка принята.');
          $('.send-ring-button').show();
          $('.loading-send-ring').remove();
          closeModal($('.wrapper-modal-back-ring'));
        } else {
              $('span.txtopermsg').css('display','inline-block');
          $('span.txtopermsg').text('Сбой отправки заявки в сети.');
          $('.send-ring-button').show();
          $('.loading-send-ring').remove();
          closeModal($('.wrapper-modal-back-ring'));
        }
